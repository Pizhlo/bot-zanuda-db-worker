...
// UpdateNotes обновляет заметки.
func (s *UnitOfWork) UpdateNotes(ctx context.Context, notes []interfaces.Message) {
	id := random.String(10)
	...
	if err := s.execWithRollback(ctx, id, func() error {
		return s.Repos.Postgres.UpdateNotes(ctx, id, notes)
	}); err != nil {
