...
func (b *updatePostgresBuilder) build() (*storage.Request, error) {
	...
	wb := newWhereBuilder().withWhere(b.where).withWhereFieldsMap(b.whereFieldsMap).withTable(b.table).withValues(b.args)
...
func (b *whereBuilder) applyAssignments() error {
	...
	for name, value := range b.args {
		if _, ok := b.whereFieldsMap[name]; ok {
			continue
		}
